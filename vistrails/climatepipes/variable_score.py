...
# We require only the synonyms module
import synonyms

#------------------------------------------------------------------------------
def match_using_synonyms( in_token, query_token ):
  """Match tokens using synonyms of query token

  Given input tokens and query tokens, match and return a score of 0 or 0.5
  """
...
def get_rank( in_str, query_str ):
  """Return a score of matching

  Given input string ( in_str ) and query string ( query_str ), calculate
  rank of in_str based on how much it is matching ( semantically ) with query_str

  This is how this algorithm works.
  - Convert strings into tokens
  - Compare tokens ( case insensitive )
  - If token from in_str matches with a token from query_str then increment
    rank by 1
  - If all the tokens from in_str matches all the tokens from query_str
    then return with a score of 1.0
